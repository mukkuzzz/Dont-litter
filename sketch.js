...
function preload(){
  ...
  lifeI = loadImage("life_image.png")
}
...
function setup() {
  ...
  life_sprite_0.addImage(lifeI);
  life_sprite_1.addImage(lifeI);
  life_sprite_2.addImage(lifeI);
  life_sprite_0.scale=0.25;
  life_sprite_1.scale=0.25;
  life_sprite_2.scale=0.25;  
  ...
  binN = createSprite(930,820,50,80);
  binN.addImage(dustbin_nondegradableI);
  binD.addImage(dustbin_degradableI)
  coingrp= new Group()
...
function draw() {
  ...
  if(gameState === 0){
    ...
    strokeWeight(10)
    stroke(188, 204, 209)
    text("Degradable Wastes:"+inv,50,50);
    text("Non-Degradable Wastes :"+nInv,50,100);
  ...
  //Checks for game end
   if(gameState===1){
    textSize(40);
    fill(255)
    text("Oops!! The garbage was left over..Better luck next time!!",400,300);
    player.destroy();
    degradableArray = [];
    nonDegradableArray = [];
    coingrp.destroyEach();
    ...
    fill(0)
    textSize(40);
    text("Your Score: " + score,1920/2-200,900/2);
